...
export const TeacherAttendance = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitMessage(uiText("Attendance for {{value0}} submitted! ({{value1}})", { value0: selectedDate, value1: summary }));
      setSubmitted(true);
      setTimeout(() => setSubmitted(false), 5000);
    } catch (err: any) {
      setSubmitMessage(uiText(err.response?.data?.error?.message || 'Failed to submit attendance'));
      setSubmitted(true);
      setTimeout(() => setSubmitted(false), 5000);
    } finally {
      setSubmitting(false);
              ...
              >
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-800 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {cls.name} {uiText(cls.section ? `• Section ${cls.section}` : '')}
                  </h3>
      ...
      <div className="relative z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 sm:p-6 rounded-3xl border border-slate-100 dark:border-slate-800/80 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex flex-col gap-3 w-full lg:w-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 justify-between lg:justify-start">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white leading-tight">
              {selectedClass.name} {uiText(selectedClass.section ? `- Section ${selectedClass.section}` : '')}
            </h2>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-between sm:justify-start">
          ...
          >
            ...
            <span>{uiText(submitting ? 'Submitting...' : 'Submit Attendance')}</span>
          </button>
        </div>
      </div>
                  ...
                  filteredStudents.map((student) => {
                    ...
                    return (
                      <tr key={student.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-800/20 transition-colors duration-200">
                        ...
                        <td className="px-6 py-4.5">
                          <span className="font-mono text-xs bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-slate-600 dark:text-slate-400 font-semibold">{uiText(student.digitalId)}</span>
                        </td>
                        <td className="px-6 py-4.5">
