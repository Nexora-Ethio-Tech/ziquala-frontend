                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800 dark:text-white">
                      {(generationError ? uiText('Generation Failed') : (isApprovedRun ? uiText('Active Approved Timetable') : uiText("{{value0}} Candidate{{value1}} Generated", {value0: generationResult!.candidateCount, value1: generationResult!.candidateCount !== 1 ? 's' : ''})))}
                    </h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mt-0.5">
                      {(generationError ? uiText('Review constraints and try again') : (isApprovedRun ? uiText('Currently published and active for all teachers and classes') : uiText('Select a timetable option to approve and publish')))}
                    </p>
                  </div>
                </div>
                <ChevronDown
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div className={`w-10 h-10 rounded-xl font-black flex items-center justify-center text-lg ${
                                ...
                                {(isApprovedRun ? uiText('✓') : uiText(realIdx + 1))}
                              </div>
                              <div>
                                <div className="flex items-center gap-2">
                                  <h4 className="font-black text-slate-800 dark:text-white">
                                    {(isApprovedRun ? uiText('Approved Timetable') : uiText("Option {{value0}}", {value0: realIdx + 1}))}
                                  </h4>
                                  {(candidate.isApproved || isApprovedRun) && (
                                    <span className="px-2.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 text-[10px] font-black uppercase rounded-full tracking-wider border border-emerald-300 dark:border-emerald-700">{uiText(" Approved & Published ")}</span>
                                  )}
                                </div>
                                <p className="text-xs text-slate-400 font-bold">
                                  {candidate.slotsFilled}/{candidate.totalSlots}{uiText(" slots filled • ")}{uiText(candidate.fillRate)}{uiText(" coverage ")}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              {candidate.isApproved || isApprovedRun ? (
                                <span className="flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-4 py-2 rounded-xl font-black text-xs uppercase tracking-wider">
                                  <CheckCircle2 size={16} />{uiText(" Active Timetable ")}</span>
                              ) : (
                                <button
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
                {(loadingTeachers ? uiText('...') : uiText(t('schedule.teachersCount', { count: teachers.length, defaultValue: `${teachers.length} teachers` })))}
              </span>
              <ChevronDown
                size={20}
                  ...
                  filteredTeachers.map(teacher => (
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        >
                          {(teacher.subjects || []).join(' • ') || uiText('No subjects')}
                        </p>
                      </div>
                    </button>
                  ))
