        ...
        <>
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-black mb-2 tracking-tight">{uiText("Welcome back, ")}{user?.name?.split(' ')[0]}!</h2>
              <p className="text-slate-400 font-medium">{uiText(" Digital ID: ")}<span className="text-white font-mono">{(user as any)?.digitalId || (user as any)?.digital_id || uiText('—')}</span>
              </p>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden mt-6">
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="px-3 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full text-xs font-bold">{mergedAnnouncements.length}{uiText(" announcement")}{(mergedAnnouncements.length !== 1 ? uiText('s') : uiText(''))}</span>
            </div>
                ...
                {mergedAnnouncements.map((ann: any) => {
                  ...
                  return (
                    <div key={ann.id} className="p-5 rounded-2xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-800/40 hover:bg-slate-100/60 dark:hover:bg-slate-800/70 transition-all flex flex-col justify-between shadow-sm">
                      <div>
                        <div className="flex items-center gap-2 mb-3 flex-wrap">
                          ...
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 ml-auto font-medium">
                            {(isValidDate ? uiText("{{value0}} {{value1}}", {value0: formatEthiopianLabel(ann.timestamp), value1: dateObj.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}) : uiText(''))}
                          </span>
                        </div>
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-4 pt-2 border-t border-slate-200/50 dark:border-slate-700/50 font-medium">{uiText(" Posted by: ")}{ann.posted_by_name || uiText('School Admin')}
                      </p>
                    </div>
                  );
                })}
                    ...
                    {annualPlans.map((plan: any) => (
                      <div key={plan.id} className="p-5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 mt-1">{uiText("Academic Year: ")}{plan.academic_year} · {(Array.isArray(plan.items) ? plan.items.length : 0)}{uiText(" weeks planned")}</p>
                          ...
                          <div key={s.id} className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-400 uppercase tracking-wider">{s.digitalId || s.grade || uiText('—')}</p>
                              </div>
                            </div>
                        ...
                        <>
                          ...
                          <div className="flex gap-3 pt-2">
                            ...
                            >
                              ...
                              {(isSubmittingLog ? uiText('Sending…') : uiText('Send to Parent'))}
                            </button>
                          </div>
                        </>
                      )}
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden animate-in fade-in duration-200">
              ...
              <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 p-6 rounded-3xl text-white shadow-xl mb-8 border border-blue-800/40 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-extrabold text-lg text-white uppercase tracking-tight">{uiText("Weekly Lesson Plan Calendar")}</h3>
                      {isCurrentWeek(selectedWeekDate) && (
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 tracking-wider">{uiText(" Current Week ")}</span>
                      )}
                    </div>
                    <p className="text-xs text-indigo-200/90 font-bold mt-0.5">{uiText(" Ethiopian Calendar Week: ")}{formatEthWeekRangeStr(selectedWeekDate)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 w-full lg:w-auto">
                  <div className="flex-1 lg:flex-initial flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <label htmlFor="teacher-report-week-select" className="text-xs font-black text-indigo-300 uppercase tracking-wider shrink-0 hidden sm:inline">{uiText(" Weekly Report (Week Ending): ")}</label>
                        ...
                        {getRecentWeekEndings().map((weekIso) => {
                          ...
                          return (
                            <option key={weekIso} value={weekIso} className="bg-slate-900 text-white font-bold py-1">
                              {ethLabel} {(isCurrent ? uiText('★ (Current Week)') : uiText('(Week Ending)'))}
                            </option>
                          );
                        })}
                      </select>
              ...
              {filteredMyPlans.length === 0 ? (
                <div className="bg-slate-50 dark:bg-slate-800/40 rounded-3xl p-10 border-2 border-dashed border-slate-200 dark:border-slate-700/80 text-center space-y-4">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight">{uiText("No Weekly Plans Found")}</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1 font-semibold">{uiText(" Use the \"+ Create New Plan\" or \"Submit New Plan\" buttons above to draft or submit your lesson plans. ")}</p>
                  </div>
                </div>
              ) : (
                    ...
                    >
                      <div className="space-y-3">
                        <div className="flex items-start justify-between gap-3 border-b border-slate-200 dark:border-slate-700/60 pb-3">
                          ...
                          >
                            {(plan.status === 'Approved' ? uiText('Approved ✔') : (plan.status === 'Draft' ? uiText('Draft 📝') : uiText(plan.status)))}
                          </span>
                        </div>
                        ...
                        <div className="space-y-2 text-xs">
                          {(plan.chapter_unit || plan.topic_title) && (
                            <p className="text-slate-700 dark:text-slate-300 font-bold">
                              📖 <span className="text-slate-500 dark:text-slate-400 font-medium">{uiText("Topic/Unit:")}</span> {plan.chapter_unit ? `${plan.chapter_unit} — ` : ''}{plan.topic_title || ''}
                            </p>
                          )}
                          {plan.content && (
                            <p className="text-slate-600 dark:text-slate-400 line-clamp-2">
                              <span className="font-bold text-slate-700 dark:text-slate-300">{uiText("Content:")}</span> {plan.content}
                            </p>
                          )}
                          {plan.objectives && (
                            <p className="text-slate-600 dark:text-slate-400 line-clamp-2">
                              <span className="font-bold text-slate-700 dark:text-slate-300">{uiText("Objectives:")}</span> {plan.objectives}
                            </p>
                          )}
                        ...
                        {plan.dean_feedback || plan.feedback ? (
                          <div className="bg-amber-500/10 dark:bg-amber-950/30 border border-amber-500/30 rounded-2xl p-3 text-xs space-y-1">
                            <p className="font-extrabold text-amber-700 dark:text-amber-400 uppercase text-[10px] tracking-wider">{uiText(" Department Head Feedback: ")}</p>
                      ...
                      <div className="pt-3 border-t border-slate-200 dark:border-slate-700/60 flex items-center justify-between gap-2">
                        <span className="text-[10px] font-bold text-slate-400">
                          {(plan.date ? uiText("Date: {{value0}}", {value0: plan.date.slice(0, 10)}) : uiText('Weekly Plan'))}
                        </span>
                        <div className="flex items-center gap-2">
                          <button
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden animate-in fade-in duration-200">
                  ...
                  {deptAnnualPlans.filter(p => {
                    ...
                    >
                      ...
                      <div className="space-y-1.5 text-xs">
                        <p className="text-slate-500">{uiText("Academic Year: ")}<span className="font-bold text-slate-700 dark:text-slate-300">{plan.academic_year}</span></p>
                        <p className="text-slate-500">{uiText("Weeks: ")}<span className="font-bold text-slate-700 dark:text-slate-300">{(Array.isArray(plan.items) ? plan.items.length : 0)}</span></p>
                      </div>
          ...
          <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 p-6 rounded-3xl text-white shadow-xl mb-6 border border-blue-800/40 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-extrabold text-lg text-white uppercase tracking-tight">{uiText("Weekly Academic Plan Oversight")}</h3>
                  {isCurrentWeek(selectedWeekDate) && (
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 tracking-wider">{uiText(" Current Week ")}</span>
                  )}
                </div>
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto">
              <div className="flex-1 lg:flex-initial flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                <label htmlFor="weekly-report-week-select" className="text-xs font-black text-indigo-300 uppercase tracking-wider shrink-0 hidden sm:inline">{uiText(" Weekly Report (Week Ending): ")}</label>
                <div className="relative flex-1 sm:w-72">
                  <select
              ...
              filteredDeptPlans.map((plan: any) => (
                ...
                >
                  ...
                  <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
                    ...
                    <p className="text-xs text-slate-500 font-bold uppercase mt-1">{plan.subject || uiText('—')}</p>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 w-full max-w-6xl my-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="font-black text-white text-lg tracking-tight">
                    {(editingPlan ? uiText('Edit Weekly Lesson Plan Sheet') : uiText('Weekly Lesson Plan Form'))}
                  </h3>
                </div>
              </div>
              <button
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="col-span-1 md:col-span-2">
                    <label className="text-[10px] font-black uppercase text-indigo-500 dark:text-indigo-400 block mb-1">{uiText("Target Academic Week (Week Ending)")}</label>
                    <select
                      value={(() => {
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800 flex gap-3 justify-end bg-slate-50 dark:bg-slate-800/80 rounded-b-[2rem]">
              ...
              >
                ...
                {(editingPlan ? uiText('Update Draft') : uiText('Save Draft'))}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {(editingPlan ? uiText('Submit for Review') : uiText('Submit Plan'))}
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl my-8 border border-slate-100 dark:border-slate-800">
            <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-t-2xl flex justify-between items-center z-10">
              <h3 className="text-xl font-black text-white uppercase tracking-wide">{(editingExam ? uiText('✏️ Edit Exam') : uiText('📝 Create New Exam'))}</h3>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4 max-h-[calc(100vh-300px)] overflow-y-auto">
                    ...
                    <div className="mt-1 p-3 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg space-y-2">
                      {coursesForGrade.map(course => (
                        <label key={course.id} className="flex items-center gap-2 cursor-pointer text-sm">
                          ...
                          <span className="text-slate-700 dark:text-slate-300">{course.name} ({uiText(course.code)})</span>
                        </label>
                      ))}
                    </div>
                  ) : examForm.gradeId ? (
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  {(examForm.questions.length === 0 ? uiText('No questions added yet. This feature will be available in the full version.') : uiText("{{value0}} questions configured", {value0: examForm.questions.length}))}
                </p>
              </div>
            </div>
            ...
            <div className="sticky bottom-0 bg-slate-50 dark:bg-slate-800 p-6 rounded-b-2xl flex gap-3 border-t border-slate-100 dark:border-slate-700">
              ...
              <button onClick={handleSaveExamChanges} disabled={submitting}
                ...
                {(editingExam ? uiText('Update Exam') : uiText('Save Exam'))}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto print:max-h-none print:overflow-visible print:p-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-800/40 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 print:bg-white print:border-slate-400">
                <div className="border border-dashed border-slate-300 dark:border-slate-700 p-3 rounded-xl text-center print:border-solid print:border-slate-400">
                  ...
                  <p className="font-bold text-slate-800 dark:text-white text-xs mt-2">{selectedPlanForView.teacher_name || selectedPlanForView.teacherName || uiText('Assigned Teacher')}</p>
                  <p className="text-[10px] text-slate-400 mt-1">{uiText("Date: ")}{(selectedPlanForView.created_at ? new Date(selectedPlanForView.created_at).toLocaleDateString() : (selectedPlanForView.date || uiText('—')))}</p>
                </div>
                <div className="border border-dashed border-slate-300 dark:border-slate-700 p-3 rounded-xl text-center print:border-solid print:border-slate-400">
                  <p className="text-[10px] font-black uppercase text-slate-400">{uiText("Department Head Signature & Date")}</p>
                  <p className="font-bold text-slate-800 dark:text-white text-xs mt-2">{(selectedPlanForView.status === 'Approved' ? uiText('Verified & Approved') : uiText('Pending Approval'))}</p>
                  <p className="text-[10px] text-slate-400 mt-1">{uiText("Date: ")}{(selectedPlanForView.updated_at ? uiText(new Date(selectedPlanForView.updated_at).toLocaleDateString()) : uiText('—'))}</p>
                </div>
                <div className="border border-dashed border-slate-300 dark:border-slate-700 p-3 rounded-xl text-center print:border-solid print:border-slate-400">
                  <p className="text-[10px] font-black uppercase text-slate-400">{uiText("Principal / VP Signature & Date")}</p>
                  <p className="font-bold text-slate-800 dark:text-white text-xs mt-2">{(selectedPlanForView.status === 'Approved' ? uiText('Signed for Academic Oversight') : uiText('Awaiting Review'))}</p>
                  <p className="text-[10px] text-slate-400 mt-1">{uiText("Date: ")}{(selectedPlanForView.updated_at ? uiText(new Date(selectedPlanForView.updated_at).toLocaleDateString()) : uiText('—'))}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-white uppercase tracking-tight">
                    {(editingAnnualPlan ? uiText('Edit Annual Plan') : uiText('New Yearly Lesson Plan'))}
                  </h3>
