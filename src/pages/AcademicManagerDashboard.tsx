...
const workspaces = [
  ...
  { title: 'eLearning', description: 'Publish Drive books by grade, subject, and shared collection.', path: '/elearning-library', icon: LibraryBig },
...
export const AcademicManagerDashboard = () => (
  <div className="space-y-8 pb-12">
    <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-emerald-950 via-school-primary to-emerald-700 p-8 text-white shadow-2xl md:p-10">
      <div className="max-w-3xl">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.25em]">
          <BarChart3 size={14} />{uiText(" Academic leadership")}</div>
    ...
    <section>
      <div className="mb-5">
        <p className="text-xs font-black uppercase tracking-[0.24em] text-school-primary">{uiText("Academic operations")}</p>
        <h2 className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{uiText("Your oversight workspaces")}</h2>
      </div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {workspaces.map((workspace) => (
          <Link key={workspace.title} to={workspace.path} className="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-school-primary/30 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <h3 className="text-lg font-black text-slate-900 dark:text-white">{uiText(workspace.title)}</h3>
            <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{uiText(workspace.description)}</p>
            <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-school-primary">{uiText("Open workspace ")}<ArrowRight size={15} className="transition-transform group-hover:translate-x-1" />
            </span>
          </Link>
        ))}
      </div>
